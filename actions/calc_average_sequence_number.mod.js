...
const OPERATION_FIELD_CONTAINER_MAPPINGS = {
    average: [],
    averageBy: ["selectorContainer"],
    averageByData: ["dataSelectorContainer"]
};

/** @type {Record<keyof typeof OPERATIONS, string> }} */
const OPERATION_HELP_TEXTS = {
    average: /* html */ `
    Stores the average of all numbers in the sequence or <span class="help_highlightText">NaN</span> if the sequence is empty.
    `,
    averageBy: /* html */ `
    Stores the average of all numbers specified by the given <span class="help_highlightText">Selector Function</span> or <span class="help_highlightText">NaN</span> if the sequence is empty.
    <br><br>
    <b>Options</b>
    ...
    `,
    averageByData: /* html */ `
    Stores the average of all numbers associated to the given <span class="help_highlightText">Data Name</span> or <span class="help_highlightText">NaN</span> if the sequence is empty.
    <br><br>
    <b>Options</b>
    <div class="help_textContainer">
        <table class="options-table">
...
/** @type {DBMAction} */
module.exports = {
    name: "Calculate Average Sequence Number [DBM German]",
    displayName: "Calculate Average Sequence Number",
    section: "Sequences",

    subtitle(data, presets) {
        return /* html */ `Calculate the Average of Numbers in ${presets.getVariableText(data.storage, data.varName)}`;
    },
        ...
        <div>
            <div style="float: left; width: 45%;">
                <span class="dbminputlabel">
                    <span>Operation</span>
                    <help-icon dialogTitle="Terminal Operations Help" dialogWidth="600" dialogHeight="570" type="0">
                        <style>
                            table.options-table {
                                width: 100%;
    ...
    action(cache) {
        ...
        try {
            switch (operation) {
                case "average":
                    result = sequence.average();
                    break;
                case "averageBy":
                    result = sequence.map(selectorFn).average();
                    break;
                case "averageByData":
                    result = sequence.map(item => item?.data?.(dataName, dataDefaultVal));
                    break;
            }
        } catch (e) {
